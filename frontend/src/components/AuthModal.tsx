...
//Компонент PasswordInput приймає та показує помилку
const PasswordInput: React.FC<PasswordInputProps> = ({ id, error, ...props }) => {
...
export function AuthModal({
  ...
  const validationErrors = useMemo(() => {
    ...
    if (mode === "register") {
      if (formData.firstName.length > 0 && formData.firstName.length < 3)
        errors.firstName = "Ім’я повинно містити мінімум 3 літери";
      
      if (formData.lastName.length > 0 && formData.lastName.length < 3)
        errors.lastName = "Прізвище повинно містити мінімум 3 літери";
      
      if (formData.phone.length > 4 && !/^\+380\d{9}$/.test(formData.phone))
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (digits.length <= 9) {
      handleInputChange("phone", `+380${digits}`);
    }
  };
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const regex = /^[a-zA-Zа-яА-ЯіїєґІЇЄҐ'-]*$/;
                        if (regex.test(value)) {
          ...
          <DialogHeader>
            <DialogTitle className="text-red-600 flex flex-col items-center justify-center">
              <svg 
                ...
                className="lucide lucide-circle-alert h-20 w-20 text-yellow-400 fill-yellow-400 stroke-white mb-2" // Заповнюємо жовтим, обведення білим для знака оклику
              >
