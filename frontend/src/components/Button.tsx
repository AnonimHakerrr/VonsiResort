	...
	"inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
	{
		variants: {
			variant: {
				...
				outline:
					'border bg-background shadow-xs text-black hover:!bg-white hover:!text-black dark:bg-input/30 dark:border-input dark:hover:bg-input/50',
				secondary:
				...
				// ← твої кастомні кнопки
				login:
					'bg-black text-white border-2 border-yellow-400 rounded-lg hover:bg-yellow-400 hover:text-black transition-colors',
				register:
					'bg-yellow-400 text-black rounded-lg hover:bg-yellow-500 transition-colors',
			},
			size: {
	...
	return (
		<Comp
			...
			className={cn(
				...
				className,
				'block w-full' // щоб кнопка займала всю ширину контейнера
			)}
			{...props}
		/>
	)
